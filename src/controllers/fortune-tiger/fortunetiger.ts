...
export default {
   async getiger(req: Request, res: Response) {
      try {
         ...
         if (jsonprimay.length === 0 || jsonprimay[0].json === null) {
         ...
         logger.error(error);
         res.status(500).send({ err: "Internal Server Error", message: error.message || error });
      }
   },
   async spin(req: Request, res: Response) {
      ...
      try {
         ...
         try {
            ...
            console.warn("[CALLBACK ERROR] Fallback to local database balance:", e.message);
         }
         ...
         if (resultadospin.result === "perda") {
            ...
            try {
               ...
               console.warn("[CALLBACK ERROR] Debit failed, continuing locally:", e.message);
            }
            res.send(json)
         }
         if (resultadospin.result === "ganho") {
            ...
            try {
               ...
               console.warn("[CALLBACK ERROR] Win callback failed, continuing locally:", e.message);
            }
            res.send(json)
         }
         if (resultadospin.result === "bonus" && resultadospin.gamecode === "fortune-tiger") {
            ...
            if (calltwo[0].steps === 0) {
               ...
               try {
                  ...
                  console.warn("[CALLBACK ERROR] Bonus callback failed, continuing locally:", e.message);
               }
               res.send(json)
               return false
            }
