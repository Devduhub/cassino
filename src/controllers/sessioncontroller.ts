...
export default {
   async verifySession(req: Request, res: Response) {
      ...
      console.log(`[VERIFY SESSION] Started - Token: ${token}, GI: ${gi}`);
      try {
         const user = await allfunctions.getuserbytoken(token)
         console.log(`[VERIFY SESSION] User lookup finish - Found: ${!!user[0]}`);
         ...
         console.log(`[VERIFY SESSION] Session verified successfully for user: ${user[0].username}, Game: ${gamename}. Sending response.`);
         res.send({
            dt: {
               oj: { jid: 0 },
               pid: "bL0qYlMgZs",
   ...
   async gamename(req: Request, res: Response) {
      const gi = req.body.gi
      console.log(`[GAMENAME/GET] Request for GI: ${gi}`);
      ...
      console.log(`[GAMENAME/GET] Response for GI ${gi}:`, JSON.stringify(response));
      res.send(response);
   },
